...
class ProdutoController extends Controller
{
    ...
    /**
     * Listar produtos públicos de uma empresa específica (para clientes)
    ...
    public function listarPorEmpresa(Request $request, string $empresaSlug)
    {
        ...
        if (!$empresa) {
            return response()->json([
                'success' => false,
                'message' => 'Empresa não encontrada ou não disponível.'
            ], 404);
        }
        ...
        if ($request->has('destaque') && $request->boolean('destaque')) {
        ...
        // Ordenação - padrão por nome, mas pode ser por preço
        $orderBy = $request->get('order_by', 'nome');
        $orderDirection = $request->get('order_direction', 'asc');

        // Validar campos de ordenação permitidos
        $allowedOrderBy = ['nome', 'preco', 'created_at'];
        ...
        // Paginação otimizada para infinite scroll
        $perPage = $request->get('per_page', 20); // Mais produtos por página para infinite scroll
        $produtos = $query->paginate($perPage);
